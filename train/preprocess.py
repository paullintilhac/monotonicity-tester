...
with strat.scope():
    with tf.Session() as sess:
        ...
        def testBatch(x,xgb_mod=None,cap=None,centered=True,path=False,all_neighbors=False,p=None):
        ...
        with open("tests/"+output_folder+"/"+filename+"_"+D+pathString+trainString+'.csv', 'w', newline='') as file:
            ...
            for e,d,m in sorted_by_m:
                ...
                if D=="empirical":
                    if not lastSuccess:
                        success="Reject"
                    elif (train_only and rollingM>1608835) or (not train_only and rollingM>590000): #these are lower bounds on the number of unique pairs for test, train
                        success = "N/A"
                    else:
                        success = testBatch(x_test,cap=m,xgb_mod=xgb_model,centered=False,path =path,p=pairs)
