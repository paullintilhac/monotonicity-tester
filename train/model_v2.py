...
class Model(object):
    def __init__(self):
        self.x_input = tf.compat.v1.placeholder(tf.float32, shape = [None, 3514])
        self.y_input = tf.compat.v1.placeholder(tf.int64, shape = [None])

        h_fc1 = tf.compat.v1.layers.dense(self.x_input, 200, activation=tf.nn.relu, name="fc1")
        h_fc2 = tf.compat.v1.layers.dense(h_fc1, 200, activation=tf.nn.relu, name="fc2")

        self.pre_softmax = tf.compat.v1.layers.dense(h_fc2, 2, activation=None, name="pre_softmax")
        ...
        _, self.accuracy_op =\
                        tf.compat.v1.metrics.accuracy(labels=self.y_input,\
                        predictions=self.y_pred)
        
        _, self.false_positive_op = tf.compat.v1.metrics.false_positives(self.y_input, self.y_pred) 


        self.weights = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES)
    ...
    def tf_interval1(self, batch_size):
        self.upper_input = tf.compat.v1.placeholder(tf.float32, shape = [None, 3514])
        self.lower_input = tf.compat.v1.placeholder(tf.float32, shape = [None, 3514])
        ...
        center = tf.compat.v1.layers.dense(inputs=center, units=200, activation=None,\
                                                name="fc1", reuse=True, use_bias = False)
        equation = tf.compat.v1.layers.dense(inputs=equation, units=200, activation=None,\
                                                name="fc1", reuse=True, use_bias = False)
        bias_row = tf.compat.v1.layers.dense(inputs=bias_row, units=200, activation=None,\
                                                name="fc1", reuse=True)
        error_row = tf.compat.v1.layers.dense(inputs=error_row, units=200, activation=None,\
                                                name="fc1", reuse=True, use_bias = False)
        ...
        center = tf.compat.v1.layers.dense(inputs=center, units=2, activation=None,\
                                                name="pre_softmax", reuse=True, use_bias = False)
        equation = tf.compat.v1.layers.dense(inputs=equation, units=2, activation=None,\
                                                name="pre_softmax", reuse=True, use_bias = False)
        bias_row = tf.compat.v1.layers.dense(inputs=bias_row, units=2, activation=None,\
                                                name="pre_softmax", reuse=True)
        error_row = tf.compat.v1.layers.dense(inputs=error_row, units=2, activation=None,\
                                                name="pre_softmax", reuse=True, use_bias = False)
